...
import { blue, green, red, yellow } from "https://deno.land/std@0.163.0/fmt/colors.ts";
import { copyN } from "https://deno.land/std@0.163.0/io/util.ts";
import { basename, join as joinPath } from "https://deno.land/std@0.163.0/path/mod.ts";
...
const flagOutputLimit = flag("output-limit", "truncate subcommand output after this many bytes", 100*1024*1024);
...
function printUsageAndExit(exitCode: number): never {
    ...
    console.log("");
    console.log("Options:");
    const longestName = Math.max(...Array.from(flags.values()).map(f => f.name.length));
    const longestDesc = Math.max(...Array.from(flags.values()).map(f => f.desc.length));
...
if (filename == null) {
    log.notice("No filename provided, trying to find a workflow file (use --workflow)");
...
if (!jobname) {
    ...
    if (jobnames.length === 1) {
        ...
        log.notice(`Multiple jobs found in ${filename}, please specify one (or use --job)`);
...
for (const [i, step] of job.steps.entries()) {
    ...
    if (step.run) {
        log.brief(bullet, green("_"), green(step.name || "<unnamed>"));
        log.brief(indent, green("$"), green(step.run));
    } else if (step.uses) {
        log.brief(bullet, red("X"), step.name || "<unnamed>", `(${yellow(step.uses)})`);
        if (step.with) {
...
for (const [i, step] of job.steps.entries()) {
    ...
    if (!step.run) {
        log.brief(stepID, step.name || "<unnamed>", yellow("skipped"), `(can't run ${step.uses})`);
